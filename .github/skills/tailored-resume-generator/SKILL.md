...
When no specific job description is available:

```
Please optimize my existing resume for general applications as a [role/level].
Focus on: impact quantification, action verbs, professional summary, and consistency.
```

## Instructions

When a user requests resume tailoring or optimization:
...
- **LaTeX / Awesome-CV**: Use project-specific macros when editing source files
...
- Standardize date formats across all entries (e.g., always `Mon YYYY -- Mon YYYY`)
...
- Use consistent verb tense (past tense for past roles, present for current role)
- Standardize formatting (dates, dashes, bullet spacing)
...
- Mix present and past tense within bullets for the same role
